...
- **运行时直接用编译键位**：`keymap.c` 用强符号覆盖了 `keymap_key_to_keycode()`，改为调用 `keycode_at_keymap_location_raw()`，即**忽略 EEPROM 里的 VIA 动态键位、直接从固件 `keymaps` 取键**。这是让 Fn 对调/音量等编译键位生效的根本手段。
- 另加：每次刷入**新编译**的固件后首次开机，会用完整 `QMK_BUILDDATE`（含时分秒）哈希与 EEPROM 中记录的构建标识比较，不同则 `dynamic_keymap_reset()` 把编译键位重灌进 VIA 动态键位（保持 VIA 侧数据一致）。VIA 自带 magic 只用日期的年/月/日，同一天多次编译不会触发重置。
